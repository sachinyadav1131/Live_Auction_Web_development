...
const SignUp = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://india-bank-search.sameerkumar.website/api/banks")
      .then((res) => {
        if (!res.ok) throw new Error("API error");
        return res.json();
      })
      .then((data) => {
        const names = Array.from(new Set(data.map((b) => b.BANK))).sort();
        setBanks(names.length > 0 ? names : staticBanks);
      })
      .catch((err) => {
        console.error("API fetch failed. Using static list.", err);
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                <label className="text-[16px] text-stone-600 font-semibold">
                  Account Number And Paypal Details
                </label>
                <div className="flex flex-col gap-1 sm:flex-row sm:gap-4">
                  <input
                    type="number"
